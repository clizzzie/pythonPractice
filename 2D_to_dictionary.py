...
def main():
    ...
    Input   : [1] GCtxt is the 2D array document.
            : [2] saveDocument is the name of the new dictionary as a json file
    Output  : A new text document will appear in the same directory as savedDocument
    """

    GCtxt           = sys.argv[1]
    saveDocument    = sys.argv[2]

    assert type(GCtxt)        == str           , 'GCtxt is a string'
    ...
    GC_dict_str               = ndarray_to_string(GCdict, GClist)
    print_stats(GCdict)

    with open(saveDocument, 'w') as write_file:
        json.dump(GC_dict_str, write_file)
